...
class PreTrainer(object):
    ...
    def load_model(self):
        ...
        if os.path.exists(self.args.save_path):
            ...
            if last_epoch > 0:
                self.model.encoder.load_state_dict(torch.load(osp.join(self.args.save_path, 'epoch%d.pth' % last_epoch))['params'])
                if torch.cuda.is_available():
                    torch.backends.cudnn.benchmark = True
                    self.model = self.model.cuda()
                print("start pretrain with epoch %d" % last_epoch)
        return last_epoch + 1
    ...
    def train(self):
        ...
        # Start pretrain
        for epoch in range(last_epoch, self.args.pre_max_epoch + 1):
