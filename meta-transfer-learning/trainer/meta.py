...
class MetaTrainer(object):
    ...
    def __init__(self, args):
        ...
        self.model_encoder_dict = self.model.encoder.state_dict()
        ...
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in self.model.encoder.state_dict()}
        self.model_encoder_dict.update(pretrained_dict)
        # self.model.load_state_dict(self.model_dict)

        self.model.encoder.load_state_dict(self.model_encoder_dict)
    ...
    def load_model(self):
        ...
        if os.path.exists(self.args.save_path):
            pattern = re.compile('epoch(\d*).pth')
            for filename in os.listdir(self.args.save_path):
                if re.match(pattern, filename):
                    last_epoch = max(last_epoch, int(pattern.findall(filename)[0]))
            if last_epoch > 0:
                self.model.load_state_dict(torch.load(osp.join(self.args.save_path, 'epoch%d.pth' % last_epoch))['params'])
                if torch.cuda.is_available():
                    torch.backends.cudnn.benchmark = True
                    self.model = self.model.cuda()
                print("start training with epoch %d" % last_epoch)
        return last_epoch + 1
    ...
    def train(self):
        ...
        # Start meta-train
        for epoch in range(last_epoch, self.args.max_epoch + 1):
    ...
    def eval(self):
        ...
        if self.args.eval_weights is not None:
            ...
            self.model.encoder.load_state_dict(torch.load(osp.join(self.args.save_path, 'max_acc' + '.pth'))['params'])
